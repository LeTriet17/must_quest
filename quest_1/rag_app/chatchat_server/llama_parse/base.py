...
class LlamaParse(BasePydanticReader):
    ...
    result_type: List[ResultType] = Field(
    ...
    async def _get_job_result(
        self, job_id: str, result_types: Union[str, List[str]], verbose: bool = False
    ) -> dict:
        result_urls = [f"{self.base_url}/api/parsing/job/{job_id}/result/{result_type}" for result_type in result_types]
            ...
            async with httpx.AsyncClient(timeout=self.max_timeout) as client:
                ...
                if status == "SUCCESS":
                    ...
                    for result_url in result_urls:
                        ...
                        parsed_results.append(result.json())
    ...
    async def _aload_data(
        ...
        try:
            ...
            results = await self._get_job_result(
                job_id, [result_type.value for result_type in self.result_type], verbose=verbose
            ) 

            # Chnage key of results pages to json
            results[-1]['json'] = results[-1]['pages']
            # del results[-1]['pages']
            documents = []
            for result in results:
                for result_type in self.result_type:
                    if result_type.value in result:
                        if result_type == ResultType.JSON:
                            json_data = [
                                {
                                    page_content['page']: {
                                    "table": [item['csv'] for item in page_content['items'] if item['type'] == 'table'],
                                    "md": page_content['md'],
                                }
                                }
                                for page_content in result[result_type.value]
                            ]
                            result['json'] = json.dumps(json_data)
                        documents.append(Document(page_content=result[result_type.value], metadata=extra_info or {}))
            return documents
    ...
    async def get_json_result(
        ...
        try:
            return await self.aget_json(file_path, extra_info)
        except RuntimeError as e:
            if nest_asyncio_err in str(e):
                raise RuntimeError(nest_asyncio_msg)
            else:
